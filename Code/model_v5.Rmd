...
xreg_fc_3 <- cbind(
  tail(full_daily$daily_avg_temp, horizon),  # Last 59 days of temperature
  fourier(ts_full, K = K3, h = horizon)     # Fourier terms for 59-day forecast
)
...
# Defining forecast horizon and dates
final_dates <- seq(as.Date("2011-01-01"), as.Date("2011-02-28"), by = "day")
horizon <- length(final_dates)
# Final Forecast df
...
fc_arima_final <- forecast(fit_arima_full, h = horizon)
fc_nnar_resid_final <- forecast(fit_nnar_resid_full, h = horizon)
